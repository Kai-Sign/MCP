...
async function startMetadataApi() {
  const files = new Map([
    ...
    [accountA.toLowerCase(), '../metadata/aa/alchemy-light-account-v2.json'],
    [accountB.toLowerCase(), '../metadata/aa/alchemy-light-account-v2.json'],
    [usdc.toLowerCase(), '../metadata/tokens/usdc.json'],
    [dai.toLowerCase(), '../metadata/tokens/dai.json'],
    [universalRouter.toLowerCase(), '../metadata/protocols/uniswap-universal-router.json']
  ]);
...
const client = new Client({ name: 'real-aa-entrypoint-router-batch-test', version: '1.0.0' }, { capabilities: {} });
...
try {
  const result = await client.callTool({ name: 'clear_sign_payload', arguments: tx });
  ...
  console.log(JSON.stringify({
    ...
    nestedCalls: parsed.clearSign.nestedCalls?.map(c => ({
      target: c.target,
      selector: c.selector,
      functionName: c.functionName,
      intent: c.intent,
      success: c.success,
      decodedCommands: c.decodedCommands?.map(cmd => ({ command: cmd.command, name: cmd.name, intent: cmd.intent }))
    })),
